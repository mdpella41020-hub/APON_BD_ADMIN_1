...
def _run_async(coro):
    ...
    asyncio.set_event_loop(loop)          # ← required so JwtGen's internal get_event_loop() works in threads
    try:
...
def badge_squad_worker(target_uid, badges_str, fast_mode):
    ...
    while active_spam_targets.get(target_uid, {}).get("running", False):
        ...
        if not accounts:
            _add_log(target_uid, "❌ [M1+M2] account.txt empty or missing — Badge/Squad disabled", "error")
            time.sleep(5)
            continue
        ...
        for bot_uid, bot_pass in accounts:
            ...
            try:
                ...
                if not result:
                    _add_log(target_uid, f"❌ [M1+M2] Auth failed for bot: {bot_uid}", "error")
                    continue

                key, iv, online_ip, online_port, auth_token = result
                _add_log(target_uid, f"✅ [M1+M2] Bot authed: {bot_uid} → {online_ip}:{online_port}", "info")

                sock = _connect_tcp(online_ip, online_port, auth_token)
                if not sock:
                    _add_log(target_uid, f"❌ [M1+M2] TCP connect failed: {online_ip}:{online_port}", "error")
                    continue
                ...
                try:
                    ...
                    for badge_key in badges_list:
                        ...
                        try:
                            ...
                            _add_log(target_uid, f"   ⚠️ Badge pkt error ({badge_label}): {be}", "error")
                        time.sleep(delay_b)

                    if badge_sent == 0:
                        _add_log(target_uid, f"⚠️ [M1] No badge packets sent for {bot_uid}", "error")
                    ...
                    try:
                        ...
                        if sent:
                            ...
                            _add_log(target_uid, f"⚠️ [M2] Squad packets built but none sent for {bot_uid}", "error")
                        else:
                            _add_log(target_uid, "⚠️ [M2] xC4 not found — Squad Invite skipped", "error")
                    except Exception as se:
                        _add_log(target_uid, f"❌ [M2] Squad error: {se}", "error")
                ...
                _add_log(target_uid, f"❌ [M1+M2] Unexpected error ({bot_uid}): {e}", "error")
